    ...
    <section id="pricing" className="py-20 lg:py-28 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16">
        <div className="text-center mb-12 lg:mb-16">
          ...
          <p className="text-(--text3) max-w-2xl mx-auto mb-8 text-sm lg:text-base">{t('landing.pricing.description')}</p>

          {/* Toggle */}
          <div className="inline-flex items-center gap-3 bg-(--bg3) border border-(--border2) rounded-full p-1.5">
            <button
              onClick={() => setAnnual(false)}
              className={`px-5 lg:px-7 py-2.5 rounded-full text-sm lg:text-base font-medium transition-all ${!annual ? 'bg-[#2354F4] text-white' : 'text-(--text3) hover:text-(--text)'}`}
            >
            ...
            <button
              ...
              className={`px-5 lg:px-7 py-2.5 rounded-full text-sm lg:text-base font-medium transition-all ${annual ? 'bg-[#2354F4] text-white' : 'text-(--text3) hover:text-(--text)'}`}
            >
              {t('landing.pricing.annual')} <span className="text-emerald-400 text-xs lg:text-sm font-bold ml-1">{t('landing.pricing.save')}</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto">
          {plans.map((plan) => (
            <div
              key={plan.name}
              className={`relative glass-card rounded-2xl p-6 lg:p-8 transition-all duration-300 hover:-translate-y-1 ${
                plan.popular ? 'border-[#2354F4] shadow-[0_0_30px_rgba(35,84,244,0.15)]' : 'border border-(--border)'
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 lg:px-6 lg:py-1.5 bg-linear-to-r from-blue-600 to-indigo-600 rounded-full text-xs lg:text-sm font-bold text-white">
                  Most Popular
                </div>
              )}

              <div className="flex items-center gap-3 mb-4 lg:mb-6">
                <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl ${plan.color === 'purple' ? 'bg-purple-500/15' : plan.color === 'blue' ? 'bg-blue-500/15' : 'bg-(--bg3)'} flex items-center justify-center`}>
                  <plan.icon className={`w-5 h-5 lg:w-6 lg:h-6 ${plan.color === 'purple' ? 'text-purple-400' : plan.color === 'blue' ? 'text-blue-400' : 'text-(--text2)'}`} />
                </div>
                <div>
                  <h3 className="text-(--text) font-bold text-lg lg:text-xl">{plan.name}</h3>
                  <p className="text-(--text2) text-xs lg:text-sm">{plan.desc}</p>
                </div>
              </div>

              <div className="mb-6 lg:mb-8">
                <span className="text-3xl lg:text-4xl font-bold text-(--text)">
                  {annual ? plan.price.annual : plan.price.monthly}
                </span>
                {plan.period && (
                  <span className="text-(--text3) text-sm lg:text-base ml-1">{annual ? plan.period.annual : plan.period.monthly}</span>
                )}
              </div>
              ...
              <button className={`w-full py-3 lg:py-3.5 text-sm lg:text-base ${
                plan.popular
